<?php 
	...
	if(empty($_SESSION['_id'])){
		...
		$smarty->assign("head_title",'文章类目创建'.WEB_NAME);
		...
		if("create"==$_REQUEST['operator'] || "update"==$_REQUEST['operator']){
			...
			if("create"==$_REQUEST['operator']){
				$ret = createNewsMenu($menu);
			}else{
				$ret = updateNewsMenu($menu);
			}
			...
			echo "window.location.href='news_type.php';";
			echo '</script>';
		}
		else{
			...
			$smarty->display('templates/news_type_create.tpl',md5($_SERVER["REQUEST_URI"]));
		}
