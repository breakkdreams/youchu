<?php 
	...
	if(empty($_SESSION['_id'])){
		...
		if("delete"==$_REQUEST["cmd"] && !empty($_REQUEST["id"])){
			...
			echo "window.location='product_delete.php';";
			echo '</script>';
		}
		//2推荐操作
			...
			echo '<script>';
			echo $ret==1?"alert('保存成功');":"alert('保存失败');";
			echo "window.location='product_delete.php?page=".$_REQUEST["page"]."';";
			echo '</script>';
		}
		//3上架下架
			...
			echo '<script>';
			echo $ret==1?"alert('保存成功');":"alert('保存失败');";
			echo "window.location='product_delete.php?page=".$_REQUEST["page"]."&product_name=".myREQUEST('product_name')."';";
			echo '</script>';
		}else{
			//当前页数
			$page = 1;
			...
			if($_count>0){
				$page_info = getHtmlPageStrCN($page,$_count,'product_delete.php?page=#page#&product_name='.$params['product_name'],$page_limit);
			}else{
				$page_info='暂无相关记录';
			}
			...
			$smarty->display('templates/product_delete.tpl',md5($_SERVER["REQUEST_URI"]));
		}
