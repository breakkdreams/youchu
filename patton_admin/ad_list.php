<?php 
	...
	if(empty($_SESSION['_id'])){
		...
		$smarty->assign("head_title",'广告-列表 '.WEB_NAME);
		...
		//分页内容列表 
		$array = getADPage($page_limit,$rowid);
		...
		if($_count>0){
			$page_info = getHtmlPageStrCN($page,$_count,'ad_list.php?page=#page#',$page_limit);
		}else{
			$page_info='暂无相关记录';
		}
		...
		$smarty->display('templates/ad_list.tpl',md5($_SERVER["REQUEST_URI"]));
	}
