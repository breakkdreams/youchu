<?php 
	...
	if(!empty($_SESSION['_shop_login_id'])){
		...
		//定义允许上传的文件扩展名
		$ext_arr = array('jpg','png','JPEG','PNG');
		...
		if (in_array($filetype, $ext_arr) === true
			&& ($filesize < 50000))
		  {
		  	...
		  	else
		    {
			    ...
			    //上传目录
			    $path = "upload/comment/".$showtime."/";
			    //完整文件地址
			    $fullname = "upload/comment/".$showtime."/".$create_file;
		    	//创建目录
